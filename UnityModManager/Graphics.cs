        ...
        static class Textures
        {
            ...
            public static void Init()
            {
                ...
                foreach (var f in textureFields)
                {
                    ...
                    f.SetValue(null, tex);
                    Utils.LoadImage(tex, Convert.FromBase64String((string)stringFields.FirstOrDefault(x => x.Name == f.Name + "Base64")?.GetValue(null) ?? ""));
                }
    ...
    public static partial class Utils
    {
        ...
        public static bool LoadImage(Texture2D tex, byte[] data)
        {
            try
            {
                ...
                {
                    var LoadImage = typeof(Texture2D).GetMethod("LoadImage", new Type[] { typeof(byte[]) });
                    return (bool)LoadImage.Invoke(tex, new object[] { data });
                }
            }
            catch (Exception exception)
            {
                UnityModManager.Logger.NativeLog(exception.ToString());
                UnityModManager.Logger.Error("Error loading image");
                return false;
            }
        }
        ...
        public static Texture2D LoadTexture(string filepath)
        {
            try
            {
                ...
                {
                    var tex = new Texture2D(2, 2, TextureFormat.ARGB32, true, true);
                    tex.name = Path.GetFileNameWithoutExtension(filepath);
                    if (LoadImage(tex, System.IO.File.ReadAllBytes(filepath)))
                        return tex;
                }
                else
                {
                    UnityModManager.Logger.Log($"Unsupported file format for '{filepath}'.");
                }
            }
            catch (Exception exception)
            {
                UnityModManager.Logger.NativeLog(exception.ToString());
                UnityModManager.Logger.Error($"Error loading texture from '{filepath}'");
            }
            return null;
        }
    }
